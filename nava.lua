...
local function showForm(player)
	...
	local fs="size[36,20]"
	local batnav="N A V A L	 	B A T T L E	 	Player A"
...
--Revolution 0 Ini
	if Revolution==0 then
		...
		for i=1,15 do
			LA[i]={};RA[i]={};LB[i]={};tryA[i]={};tryB[i]={}
			for j=1,15 do LA[i][j]=0;RA[i][j]=0;LB[i][j]=0;tryA[i][j]=0;tryB[i][j]=0;end
		end
		...
		if file then
			...
			for i=1,15,1 do lina=file:read();for j=1,15,1 do LA[i][j]=tonumber(string.sub(lina,j,j));end;end;file:close()
			...
			for j=1,15,1 do LA[i][j]=tonumber(string.sub(lina,j,j));end;end;file:close()
		...
		if file then
			...
			for i=1,15,1 do
				lina=file:read()
				for j=1,15,1 do
					LB[i][j]=tonumber(string.sub(lina,j,j))
				end
			end
			file:close()
...
if showSeas==1 then
	YouCanNotDoThat=0
	fs=fs.."textlist[32,4.5;2.75,3;tlA;Player_B,"..todos..";;1]"
end
	...
	if jrlnum then
		...
		if jrlnum<(lastSeaM+1) then
			if jrlnum==1 then
				...
				--limpatryA()
				local file=io.open(minetest.get_worldpath().."/Player_B.txt","r")
				for i=1,15,1 do lina=file:read();for j=1,15,1 do RA[i][j]=tonumber(string.sub(lina,j,j));end;end;file:close()
				playingWith="button[14.5,2;5,1;v:?;Playing with Player_B]"
			else
				Aon=0;gravaAon(0)
	...
	for i=1,15,1 do
		...
		for k=1,15,1 do
			...
			if pos==nome and not jrlnum then
				if LA[i][k]==0 then
					LA[i][k]=1;bg="vermelho_color.png"
				else
					LA[i][k]=0;bg="mar_grego_color.png"
				end
			end
			fs=fs.."image_button["..((i*0.8)+dx)..","..dy..";0.8,0.8;"..bg..";v:"..nome..";]"
			bg="mar_grego_color.png"
			if LA[i][k]==1 then bg="vermelho_color.png";end
			fs=fs.."image_button["..((i*0.8)+dx)..","..dy..";0.8,0.8;"..bg..";v:"..nome..";]"
			if tryB[i][k]>0 then fs=fs.."image_button["..((i*0.8)+dx+.2)..","..(dy+.2)..";0.4,0.4;mil_color.png;v:?;]";end
		end
	end
end
...
if YouCanNotDoThat==0 then
	...
	for i=1,15,1 do
		...
		for k=1,15,1 do
			...
			if pos==nome then
				bg="mar_grego_color.png"
				if RA[i][k]==1 and tryA[i][k]==0 then bg="vermelho_color.png";minetest.sound_play("cann1");end
				if RA[i][k]==0 and tryA[i][k]==0 then minetest.sound_play("splash");end
				tryA[i][k]=1;fs=fs.."image_button["..((i*0.8)+dx)..","..dy..";0.8,0.8;"..bg..";z:"..nome..";]"
			end
			if tryA[i][k]==1 then
				bg="vermelho_color.png"
				if RA[i][k]==0 then bg="mar_grego_color.png";end
...
if jrlnum==1 and Atry>5 then
	local file=io.open(minetest.get_worldpath().."/arqtryA.txt","w")
	for i=1,15,1 do for j=1,15,1 do file:write(tryA[i][j]);end;file:write("\n");end;file:close()
	local file=io.open(minetest.get_worldpath().."/arqtryB.txt","r")
	if file then for i=1,15,1 do lina=file:read();for j=1,15,1 do tryB[i][j]=tonumber(string.sub(lina,j,j));end;end;file:close();end
	Atry=0
end
...
for i=1,15,1 do
	for j=1,15,1 do
		if RA[i][j]==1 and tryA[i][j]==1 then okA=okA+1;end
		if RA[i][j]==0 and tryA[i][j]==1 then nokA=nokA+1;end
	end
end

--opções de Player
bg="#ff0"
if okA==36 then bg="#0f0";end
if jrlnum then
	if showTank>0 then
		fs=fs.."button[19.6,2;7.9,1;v:?;"..minetest.colorize(bg,okA.." shots on Navy and "..nokA.." shots in water").."]"
	else
		fs=fs.."button[19.6,2;7.9,1;v:?;"..minetest.colorize(bg,okA.." shots on Navy and "..nokA.." shots in water").."]"
	end
end
	...
	function(player, formName, fields)
		...
		for fieldName in pairs(fields) do
			...
			if action=="v" then
				...
				pos=value
				Revolution=1
				if pos=="grav" then SaveUsA();end
				if pos=="alon" then ICallYourNameA();SaveUsNowA();pos="rese";end
				if pos=="clef" then for i=1,15 do for j=1,15 do LA[i][j]=0;end;end;end
			...
			if fieldName=="tlA" then pos="?";for i=1,15 do tryA[i]={};for j=1,15 do RA[i][j]=0;tryA[i][j]=0;end;end;jrl=fields.tlA;showForm(player);end;end
		end
	end
)
...
--Node naveA
lazznav_naveA={}
	minetest.register_node("lazznav:naveA",{
		description="naveA",
		tile_images={"naveAr.png"},
...
function gravaAon(g)
	local file=io.open(minetest.get_worldpath().."/Aon.txt","w");file:write(g);file:close()
end

function limpatryA()
	oarq=minetest.get_worldpath().."/arqtryA.txt"
